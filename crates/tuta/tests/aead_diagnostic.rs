...
fn diagnose_aead_primitives_and_legacy_dispatch() {
    ...
    let sk2 = AeadSubKeys::derive_from_group_key(
        &Versioned {
            object: generic.clone(),
            version: 7,
        },
        &nonce,
        "tutanota/97",
    ...
    for (version, subkeys, domain) in [(3, &sk3, "attributeEncSK"), (2, &sk2, "attributeEncGK")] {
        for path in ["105", "3/aggregateId/9/anotherCustomId/17"] {
            ...
            let ciphertext = aead
                .encrypt(subkeys, plaintext.clone(), aad.as_bytes())
                .unwrap();
            assert_eq!(ciphertext[0], version);
            assert_eq!(
                aead.decrypt(subkeys, &ciphertext, aad.as_bytes()).unwrap(),
                plaintext
            );
    ...
    if let Ok(path) = std::env::var("AEAD_TS_VECTORS") {
        ...
        for vector in &input {
            ...
            assert_eq!(
                aead.decrypt(subkeys, &ciphertext, aad).unwrap(),
